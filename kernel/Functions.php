...
function spRun(){
	GLOBAL $__controller, $__action;
        ...
        $__controller = ucfirst(strtolower($__controller))."Action";
        
	// 对路由进行自动执行相关操作
	spLaunch("router_prefilter");    
        //echo "<pre>";print_r(PATH_INFOS);exit;
	// 对将要访问的控制器类进行实例化
	//$handle_controller = spClass($__controller, null, $GLOBALS['G_SP']["controller_path"].'/'.$__controller.".php");
//        $handle_controller = spClass($__controller, null, $GLOBALS['G_SP']["controller_path"].trim(ucfirst(strtolower(PATH_INFOS)),"/").ucfirst(strtolower($__controller))."Action.class.php");
        $handle_controller = spClass($__controller, null, $GLOBALS['G_SP']["controller_path"].$__controller.".class.php");
...
function import($sfilename, $auto_search = TRUE, $auto_error = FALSE){
	...
	if( TRUE == @is_readable($sfilename) ){
		...
		//echo "<pre>";print_r($GLOBALS['G_SP']);exit;
		return TRUE;
	}else{
...
function spClass($class_name, $args = null, $sdir = null, $force_inst = FALSE){
    ...
    //echo "<pre>";print_r($sdir);exit;
	if(null != $sdir && !import($sdir) && !import($sdir.'/'.$class_name.'.class.php'))return FALSE;
